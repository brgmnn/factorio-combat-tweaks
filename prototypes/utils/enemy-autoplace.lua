...
-- Create a closure and insert the noise expressions for a new enemy autoplacer. This lets us
-- create different spawn points for different factions of enemies.
local function create_autoplacer(build_params)
  ...
  local function generate_enemy_base_probability_expression()
    ...
    local frequency_expression = noise.max(0, noise.var(build_params.name .. "-base-frequency"))
    local spot_radius_expression = noise.max(0, noise.var(build_params.name .. "-base-radius"))
    ...
    local blob_amplitude_multiplier = 0.2 * spot_height_expression *
                                          (0.1 + 0.9 *
                                              noise.clamp(noise.var("distance") / 3000, 0, 1))
    local blobs = tne {
      type = "function-application",
    ...
    local starting_area_hole = noise.min(0, starting_area_sharpness *
                                   (noise.var("distance") / noise.var("starting_area_radius") - 1))
  ...
  data:extend{
    {
      ...
      name = build_params.name .. "-base-intensity",
  ...
  local function enemy_autoplace(params)
    ...
    local distance_height_multiplier = noise.max(0, 1 +
                                           (distance_outside_starting_area - distance_unit *
                                               distance_factor) * 0.002 * distance_factor)

    local probability_expression = noise.var(build_params.name .. "-base-probability") *
                                       distance_height_multiplier
  ...
  local function enemy_spawner_autoplace(distance)
    return enemy_autoplace { distance_factor = distance, order = "b[enemy]-a[spawner]" }
  end
